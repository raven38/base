...
class RGBDMotionAugmentor:
    ...
    def spatial_transform(self, images, depths, poses, intrinsics, movement_maps):
        """ cropping and resizing """
        ht, wd = images.shape[2:]
        ...
        intrinsics = scale * intrinsics
        depths = depths.unsqueeze(dim=1)
        movement_maps = movement_maps.unsqueeze(dim=1)

        images = F.interpolate(images, scale_factor=scale, mode='bilinear', 
            align_corners=False, recompute_scale_factor=True)
        
        depths = F.interpolate(depths, scale_factor=scale, recompute_scale_factor=True)
        movement_maps = F.interpolate(movement_maps, scale_factor=scale, recompute_scale_factor=True)
        ...
        images = images[:, :, y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        depths = depths[:, :, y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        movement_maps = movement_maps[:, :, y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]

        depths = depths.squeeze(dim=1)
        movement_maps = movement_maps.squeeze(dim=1)
        return images, poses, depths, intrinsics, movement_maps
    ...
    def __call__(self, images, poses, depths, intrinsics, movement_maps):
        images = self.color_transform(images)
        return self.spatial_transform(images, depths, poses, intrinsics, movement_maps)
