...
from .augmentation import RGBDAugmentor, RGBDMotionAugmentor
...
class RGBDMotionDataset(data.Dataset):
    def __init__(self, name, datapath, n_frames=4, crop_size=[384,512], fmin=8.0, fmax=75.0, do_aug=True):
        """ Base class for RGBD dataset """
        self.aug = None
        self.root = datapath
        ...
        if do_aug:
            self.aug = RGBDMotionAugmentor(crop_size=crop_size)
    ...
    def movement_map_read(movement_map_file):
        return np.load(movement_map_file)
    ...
    def __getitem__(self, index):
        ...
        images_list = self.scene_info[scene_id]['images']
        depths_list = self.scene_info[scene_id]['depths']
        movement_maps_list = self.scene_info[scene_id]['movement_maps']
        poses_list = self.scene_info[scene_id]['poses']
        intrinsics_list = self.scene_info[scene_id]['intrinsics']
        ...
        for i in inds:
            ...
            poses.append(poses_list[i])
            intrinsics.append(intrinsics_list[i])
            movement_maps.append(self.__class__.movement_map_read(movement_maps_list[i]))

        images = np.stack(images).astype(np.float32)
        depths = np.stack(depths).astype(np.float32)
        poses = np.stack(poses).astype(np.float32)
        intrinsics = np.stack(intrinsics).astype(np.float32)
        movement_maps = np.stack(movement_maps).astype(np.float32)
